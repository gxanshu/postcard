...
- **`src/postcard/core/`** — UI-agnostic logic, no GTK widgets. Sub-packages: `models/` (Account, Folder, Email, Conversation, Attachment as `GObject.Object`s so `Gio.ListStore` accepts them, plus `MessageHeader` as a plain dataclass), `store/database.py` (all SQLite), `net/` (`imap_session.py`, `smtp_session.py` — thin stdlib `imaplib`/`smtplib` wrappers; `graph_session.py`, `graph_folders.py`, `graph_messages.py`, `graph_send.py` — Microsoft Graph over `urllib`; `errors.py`), `mime/message_parser.py`, plus `threader.py`, `compose.py`, `secrets.py`. This is where testable logic belongs.
...
- Folders mirror the server list each sync (`prune_folders`), keeping only the local `Outbox`. Folder role/icon/display-name classification lives in `mail_sync.py` (`role_for_folder`, `icon_for_folder`, `display_name_for_folder`) — matched by name substring, tolerant of casing and Gmail's `[Gmail]/` prefixes. `role_for_folder` returns a `FolderRole` `StrEnum` (defined in `core/models/folder.py`, re-exported by `mail_sync`); compare against its members rather than bare strings, and note that both `Archive` and Gmail's `All Mail` classify as `ARCHIVE`, which `_folder_with_role` breaks in favour of a real `Archive` folder. A folder row can also carry a `role` and `label` its server stated (Graph does; IMAP leaves both empty) — so in the GTK layer read a stored `Folder` through `mail_sync.folder_role`, `folder_label` and `is_outgoing`, never `role_for_folder(folder.name)`, which gives `OTHER` for a Graph folder id and misses a localized "Gesendete Elemente".
...
**Only OAuth accounts are imported** — Google and Microsoft 365. A GOA "Email Server" account is plain IMAP/SMTP, which the Add Account dialog already does and tests, so importing it would buy nothing but the typing; `mail_accounts()` flags it `is_oauth2` False and the dialog points at Add Account instead. A GOA "Exchange" account comes back `is_mail_supported` False, since it signs in to EWS. Neither kind is hidden, so the dialog can always say why a row is unavailable.

### Microsoft 365 goes over Graph, not IMAP

GOA's `ms_graph` provider ("Microsoft 365", which also covers personal Outlook.com accounts) hands out a token scoped to the Graph API (`Mail.ReadWrite`, `Mail.Send`) with no `IMAP.AccessAsUser.All`, so those accounts are stored with `accounts.protocol = 'graph'` and `graph.microsoft.com` in the host columns. `mail_sync`'s public functions keep their signatures and branch on `account.is_graph`, so `window.py` never knows which protocol it is talking to. The mapping, and why:

- **A Graph folder's `name` is its id**, and its role and display name are stored beside it — Graph's well-known folder names (`inbox`, `sentitems`, …) are the only reliable way to find the sent folder of a German mailbox, and an Outlook mailbox can hold both the real "Archiv" and a stray "Archive". Outbox, Sync Issues and Conversation History are hidden.
- **A message's `server_id` is its immutable id** — every request sends `Prefer: IdType="ImmutableId"`, without which an id changes on every move. It isn't numeric, so `_arrival_key` orders those by date.
- **The UID snapshot is a delta query**, kept per folder in memory, so a later sync only fetches what changed. An expired delta link restarts the walk.
- **Sending posts the finished MIME message** to `/me/sendMail`, so headers and Message-ID survive and Exchange files the Sent copy itself (no append). Graph delivers to the message's headers rather than an envelope, so Bcc recipients are written into a `Bcc` header on the copy that is sent — never on the one stored locally. A message too large for one request (413) is rebuilt as a draft with its attachments uploaded separately.
- Flag changes and moves go through `$batch`, 20 requests at a time; throttling (429/503/504) is retried after `Retry-After`.
